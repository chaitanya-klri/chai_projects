...
def extract_info_from_footer(pdf):
    for page in pdf.pages:
        # Extract the full text from the page
        page_text = page.extract_text()

        if page_text:
            # Print out the extracted text for debugging
            st.write(f"Extracted page text: {page_text}")
            
            # Adjusted regex to match just 'C' for Computational Thinking, 'E' for English, etc.
            match = re.search(r"(\d+)/([A-Z])(\d{1,2})([A-Z])", page_text.strip())
            st.write(f"Match result: {match}")  # Debug statement to check if the match is working
...
st.markdown("This facility is to be able to give you yearwise collection of skills by uploading the TMBs of a school for ASSET Pen and Paper.")
